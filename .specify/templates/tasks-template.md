...
> 任务描述必须使用自然中文，并引用精确文件路径；若需英文术语请在括号说明。
...
- [ ] TL10N [P] [Shared] 验证所有用户可见输出的中文翻译（列出文件）。
- [ ] TSTAB [Shared] 运行并记录冒烟/回归脚本（标明命令与日志位置）。
- [ ] TTEST [Shared] 为新增功能编写/更新自动化测试（注明 tests/ 文件）。
- [ ] TLINT [Shared] 执行 lint/format 命令并附带结果路径。
- [ ] TDOC [Shared] 同步更新 plan/spec/命令模板的相关段落（指向 PR/路径）。
- [ ] TSTRUCT [Shared] 确认目录结构与 plan.md 一致，若有变动需记录原因。
- [ ] TSIMP [Shared] 在 plan.md `复杂性跟踪` 中记录方案权衡，并附审查要点。
- [ ] TPERF [Shared] 验证性能/资源目标或提供后续测试计划（含负责人）。
